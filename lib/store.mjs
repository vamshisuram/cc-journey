...
export function recentGoals({ maxAgeDays = 7, limit = 5 } = {}) {
  ...
  return idx.goals
    .filter(
      (g) =>
        g.status === "active" &&
        Date.parse(g.last_active_at || g.created_at) >= cutoff,
    )
    .sort((a, b) => (b.last_active_at || "").localeCompare(a.last_active_at || ""))
    .slice(0, limit);
}

export function setGoalTitle(goalId, title) {
  const idx = readIndex();
...
export function attachSessionToGoal(goalId, sessionId) {
  const idx = readIndex();
  const g = idx.goals.find((x) => x.id === goalId);
  if (!g) return null;
  if (!g.conversation_ids.includes(sessionId)) {
    g.conversation_ids.push(sessionId);
    writeIndex(idx);
    const meta = readMeta(goalId);
    if (meta) {
      meta.conversation_ids = g.conversation_ids;
      writeMeta(goalId, meta);
    }
  }
  return g;
